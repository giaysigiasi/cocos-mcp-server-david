...
export const methods: { [key: string]: (...any: any) => any } = {
    ...
    createPrefabFromNode(nodeUuid: string, prefabPath: string) {
        try {
            ...
            // Note: This is just a simulation implementation because prefab files cannot be created directly in runtime environment
                ...
                if (typeof value === 'string') {
                    ...
                    assetManager.resources.load(value, require('cc').SpriteFrame, (err: any, spriteFrame: any) => {
                        if (!err && spriteFrame) {
                            ...
                            // Try to load via uuid
                            assetManager.loadAny({ uuid: value }, (err2: any, asset: any) => {
                                if (!err2 && asset) {
                                    component.spriteFrame = asset;
                                } else {
                                    // Direct assignment (compatible with already passed resource object)
